...
public class HumanVsAI {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Which player do you want to play as?");
        Scanner scanner2=new Scanner(System.in);
        int WhoIwantToplayAs=scanner2.nextInt();
        int turn=WhoIwantToplayAs;
        System.out.println("Choose Huristic");
        Scanner scanner=new Scanner(System.in);
        int choice=scanner.nextInt();
        while (true){
            if(turn==WhoIwantToplayAs){
                System.out.println("Turn of Player"+turn);
                Scanner scanner1 = new Scanner(System.in);
                int position = scanner1.nextInt();
                board=doMove(board,turn,position);
                printBoard(board.getPlayer1(),board.getPlayer2());
            }
            else{
                double val=alphaBeta(board,turn,-100,100,15,choice,turn);
                for(int i=0;i<children.size();i++){
                    if(val==children.get(i).getHval()){
                        board=children.get(i);
                        System.out.println("Turn of player"+turn);
                        printBoard(children.get(i).getPlayer1(),children.get(i).getPlayer2());
                        break;
                    }
                }
            }
    ...
    public static ArrayList<Board> getChildren(Board board,int turn){
    ...
    public static double evaluate(Board board,int choice,int turn){
        ...
        double H2=(13*(store1-store2)+(balls1-balls2));
        double H3;
        if(turn==1)
            H3=H2+9*extra;
        else
    ...
    public static double alphaBeta(Board board, int turn, double alpha, double beta, int depth, int choice, int who){
        ...
        if(turn==1){ //Maximize
            ...
            ArrayList<Board> boards = getChildren(board,turn);
